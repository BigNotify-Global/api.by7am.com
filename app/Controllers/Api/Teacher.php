...
class Teacher extends ResourceController
{
   ...
   public function createUpdate()
   {
      ...
      // 1. Extract and format the raw payload
      $payload = $this->extractUpdatePayload($accountId);

      // 2. Base Validation & Identity Shield
      $validationError = $this->validateUpdatePayload($accountId, $payload);
      if ($validationError)
         return $validationError;

      // 3. SaaS Limits & Temporal Calculation (The Business Engine)
      $schedulingResult = $this->enforceSaaSTierAndCalculateDates(
         $payload['schoolId'],
         $payload['profileId'],
         $payload['sectionId'],
         $payload['clientVisibleFrom'],
         $payload['clientExpiresAt'],
      );

      if (isset($schedulingResult['errorResponse'])) {
         return $schedulingResult['errorResponse']; // Returns the 403 HTTP response
      }

      // 4. Database Execution
      return $this->executeSpCreateUpdate($payload, $schedulingResult);
   }

   private function extractUpdatePayload(?string $accountId): array
   {
      ...
      return [
         ...
         'contextId' => ((int) $subjectId === 1) ? 1 : 2, // Smart Inference
   ...
   private function validateUpdatePayload(?string $accountId, array $p)
   {
      if (empty($accountId)) {
         return $this->failUnauthorized('Security violation: Verified Account ID missing.');
      }

      if (empty($p['schoolId']) || empty($p['profileId']) || empty($p['sectionId']) || trim($p['text']) === '') {
         return $this->failValidationErrors('School ID, Profile ID, Section ID, and Text are strictly required.');
      }

      if (!$this->verifyProfileOwnership((int) $accountId, (int) $p['profileId'])) {
   ...
   private function enforceSaaSTierAndCalculateDates(
      $schoolId,
      $profileId,
      $sectionId,
      $clientVisibleFrom,
      ...
      // Block basic temporal manipulation early
      if (!empty($clientVisibleFrom) && strtotime($clientVisibleFrom) < ($currentServerTime - 300)) {
         return ['errorResponse' => $this->failValidationErrors('Temporal manipulation blocked: visibleFrom cannot be in the past.')];
      }

      // Fetch School's Plan Limits
      $planQuery = $db->query("
          SELECT mp.name AS plan_name, mp.max_updates_per_day, mp.max_future_days,
              (SELECT COUNT(id) FROM updates
               WHERE source_profile_id = ? AND section_id = ?
                 AND DATE(created_at) = CURDATE() AND deleted_at IS NULL) AS updates_today
          FROM schools s
      ...
      if (!$planData)
         return ['errorResponse' => $this->failServerError('Could not verify school subscription plan.')];

      // Enforce Daily Rate Limit
      if ((int) $planData['updates_today'] >= (int) $planData['max_updates_per_day']) {
         return ['errorResponse' => $this->failForbidden("Rate limit exceeded: Your plan allows {$planData['max_updates_per_day']} updates per section per day.")];
      }

      // Enforce Future Scheduling Limit
      $maxFutureDays = (int) $planData['max_future_days'];
      if (!empty($clientVisibleFrom)) {
         if (strtotime($clientVisibleFrom) > strtotime("+{$maxFutureDays} days 23:59:59")) {
            return ['errorResponse' => $this->failForbidden("Upgrade required: The {$planData['plan_name']} plan only allows scheduling up to {$maxFutureDays} days in advance.")];
         }
      }

      // Return Calculated Safe Dates
      return [
         'visibleFrom' => !empty($clientVisibleFrom) ? $clientVisibleFrom : date('Y-m-d 06:30:00', strtotime('+1 day')),
         'expiresAt' => !empty($clientExpiresAt) ? $clientExpiresAt : date('Y-m-d 23:59:59', strtotime('+1 day'))
      ];
   }

   private function executeSpCreateUpdate(array $p, array $dates)
   {
      $db = \Config\Database::connect();
      ...
      try {
         ...
         return $this->respondCreated([
            ...
            "schedule" => ["visibleFrom" => $dates['visibleFrom'], "expiresAt" => $dates['expiresAt']],
         ]);

      } catch (\CodeIgniter\Database\Exceptions\DatabaseException $e) {
         if (strpos($e->getMessage(), '45000') !== false || strpos($e->getMessage(), 'SECURITY_VIOLATION') !== false) {
            return $this->failForbidden('Security violation: Profile does not belong to the specified school or cross-tenant action blocked.');
         }
         log_message('error', '[DB Error in executeSpCreateUpdate]: ' . $e->getMessage());
